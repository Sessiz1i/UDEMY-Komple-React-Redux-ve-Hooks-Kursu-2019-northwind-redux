...
function validate(name,value) {
	if (name === "productName" && value.length < 4 || value.length  > 10){
		setErrors(previousErrors =>({...previousErrors,productName: "Ürün ismi gereklidir"}))
	}else {
		setErrors(previousErrors =>({...previousErrors,productName:""}))
	}
}

	function hanleSave(event) {
	...
	return (
		<ProductDetail product={product} categories={categories}
					   onChange={handleChange} onSave={hanleSave} errors={errors}/>
	);
}
